...
void lcdprstr(uint16_t x, uint16_t y, const char* str,sFONT* font, uint16_t color, uint16_t bgcolor)
{
	lcdFont.pFont = font;
	while(*str) {
		         if(x + lcdFont.pFont->Width >= ILI9341_PIXEL_WIDTH) {
				            x = 0;
				            y += lcdFont.pFont->Height;
				            if(y + lcdFont.pFont->Height >= ILI9341_PIXEL_HEIGHT) {
				                break;
				            }
	            if(*str == ' ') {
	        ...
	        x += lcdFont.pFont->Width;
	        str++;
	    }
}

void lcdPrintCenteredInRect(const char* str, uint16_t rectX, uint16_t rectY, uint16_t rectWidth, uint16_t rectHeight, sFONT* font, uint16_t color, uint16_t bgcolor)
{
	lcdFont.pFont = font;
    // Lấy kích thước font
    uint16_t charWidth = lcdFont.pFont->Width;
    uint16_t charHeight = lcdFont.pFont->Height;

    // Tính độ dài chuỗi
    uint16_t strWidth = charWidth * strlen(str);

    // Tính toán tọa độ căn giữa trong hình chữ nhật
    uint16_t x = rectX + (rectWidth - strWidth) / 2;
    uint16_t y = rectY + (rectHeight - charHeight) / 2;

    // In chuỗi ra màn hình
    lcdprstr(x, y, str, font, color, bgcolor);
}
